...
Each site shows `mama@aivara.se` (and so on) in a `mailto:` link. **The repository has no part in delivering mail** — there is no form, no endpoint, no backend. The address only works once the owner has configured forwarding for `aivara.se` to a mailbox someone reads, so set that up before the site goes public.
