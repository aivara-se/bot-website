...
- One fact, one home. The authoritative documents are declared in `.agents/config.yml` — `paths.architecture`, `paths.product`, `paths.design`. Link to them; do not restate their content anywhere else, including here.
...
- Every command quoted in prose must be the command from `.agents/config.yml`; if they differ, the document is wrong.
